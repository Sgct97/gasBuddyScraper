#!/usr/bin/env python3
"""
Get ALL 34 stations with COMPLETE data extraction - FIXED
"""
...
def extract_stations_from_apollo(apollo_state):
    """Extract complete station data from Apollo state - FIXED"""
    stations = []
    ...
    for key in sorted(apollo_state.keys()):
        if key.startswith('Station:'):
            ...
            # Address is directly embedded, not a reference
            address_data = station_data.get('address', {})
            
            # Prices are also directly embedded
            prices = []
            price_reports = station_data.get('prices', [])
            for price_report in price_reports:
                if isinstance(price_report, dict) and '__typename' in price_report:
                    cash_info = price_report.get('cash')
                    credit_info = price_report.get('credit')
                    
                    price_obj = {
                        'fuel_type': price_report.get('fuelProduct', 'unknown'),
                        'cash': None,
                        'credit': None
                    }
                    
                    if cash_info and isinstance(cash_info, dict) and cash_info.get('price'):
                        price_obj['cash'] = {
                    ...
                    if credit_info and isinstance(credit_info, dict) and credit_info.get('price'):
                        price_obj['credit'] = {
...
print("="*70)
print("\nDATA SOURCES:")
print(f"  - {len(apollo_stations)} stations from Apollo state (first 20)")
print(f"  - {len(graphql_stations)} stations from GraphQL (cursor=20)")
print("\nDATA COMPLETENESS:")
